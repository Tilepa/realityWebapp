...
export class LosnummerService {
  private listOfReviews: Array<Bewertung> = [];
  private countUtilzedItems: number;
  ...
  countUtilizedByCompany( company: Unternehmen ): number {
    let temp = this.listOfReviews.filter( (item) => {
  ...
  getListOfCompanyReviews(): Array<[string, number, number, number]> {
    let listOfUtilizedCompanys: Array<[string, number, number, number]> = [];

    // Iterate over a list of companys to count the reviews
    for( let unternehmen of this.unternehmService.getListOfCompanys() ) {
      let count: number = 0;
      let realityFactor = 0;
      let attractivityFactor = 0;

      // Iterate over the list of reviews to compare and count
      for( let review of this.listOfReviews ) {
